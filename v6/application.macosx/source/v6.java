...
public void draw() {
  ...
  sinkX = width/2 + width/2 * cos(time*0.01f); //mouseX;
  sinkY = height/2 + height/2 * sin(time*0.01f);//mouseY;
  
  // sink2 is reflected across the origin from sink1
  sink2X = mouseX; //width/2 - width/2 * cos(time*0.01);
  sink2Y = mouseY; //width/2 - height/2 * sin(time*0.01);
  ...
  for (int r = 0; r < rows; r++){
    ...
    for (int c = 0; c < columns; c++){
      ...
      translate(0,0,400-0.01f*(distToSink*distToSink)+0.01f*(distToSink2*distToSink2));
  ...
  filter(GRAY);  //<-- builtin is 10x slower than customized blur code
  loadPixels();
...
public void shiftBlur(final int[] s, final int[] t, final int a, final int b, final int c){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--full-screen", "--bgcolor=#666666", "--stop-color=#cccccc", "v6" };
    if (passedArgs != null) {
